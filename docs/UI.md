...
控制页在长按时间下方显示“长按防抖”。原生菜单保留极高/高/中/低/无，中显示“中（默认）”；说明与菜单位于同一设置行内部，采用主要标签、次要说明层级，不将说明独立分隔成设置项。主说明明确容忍抖动与拖动距离的取舍。问号说明补充“无”仍可静止长按、等待时间独立、自动保存及绘画排除不受影响；关闭长按后菜单禁用，说明仍可查看。
