...
不上传私钥/证书包、导入密码、身份配置、本机路径或账户资料、原始诊断和系统日志、构建缓存、历史二进制、idea/协作记录，以及旧图标草稿。生成的 ICNS 和 DMG 不进入源码；经过审核的分发包可另行作为 Release 附件发布。
...
仅有 Git 提交或标签不会产生更新提醒，必须创建正式 GitHub Release。发布前同步 `main.swift`、构建脚本、README 和 CHANGELOG 的版本，运行测试、检查 UI、构建并验证签名及 DMG。上传源码不代表已发布安装包。

当前本地签名是开发证书签名，未做 Developer ID 公证；不可将它宣传为 Apple 公证发行版。对外分发的系统安全提示与其他设备兼容性需另行验证。
